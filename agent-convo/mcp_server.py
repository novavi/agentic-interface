...
def get_stock_data(company_name: str) -> str:
    ...
    entry = _STOCK_DATA.get(key)
    if entry is None:
        valid = ", ".join(sorted(_STOCK_DATA))
        return f"Company '{company_name}' not found. Valid options: {valid}"

    rows = "\n".join(
        f"  {month}: ${price:.2f}"
        for month, price in zip(_MONTHS, entry["prices"])
    )
    return (
        f"{entry['company']} ({entry['ticker']})\n"
        f"Monthly closing prices (Jun 2025 – May 2026):\n{rows}"
    )
