...
var LangGraphAgent$1 = class extends LangGraphAgent {
	...
	dispatchEvent(event) {
		...
		if (isToolEvent || event.type === EventType.TOOL_CALL_RESULT) {
			console.log('[CK DBG dispatchEvent] type:', event.type, 'rawEvent.event:', rawEvent?.event, 'rawEvent.name:', rawEvent?.name, 'hasEmitToolCallsFlag:', "copilotkit:emit-tool-calls" in (rawEvent?.metadata || {}), 'emitToolCallsValue:', rawEvent?.metadata?.["copilotkit:emit-tool-calls"]);
		}
